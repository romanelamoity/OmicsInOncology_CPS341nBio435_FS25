...
SuppS6 <- read_excel("science.abf3066_Table_S6.xlsx", sheet = "Differential Interaction Score")
#excel_sheets("science.abf3066_Table_S6.xlsx")
```
...
```{r}
# Extract baits and preys from the subset
baits_sub <- SuppS6_sub$Bait
preys_sub <- SuppS6_sub$Prey

# Create the PPI graph using the subset data
ppiGraph <- makeproteingraph(baits_sub, preys_sub)

# Output graph details (number of nodes and edges)
ppiGraph
```
...
ppiGraphLayout <- layoutGraph(ppiGraph, layoutType = "neato")
renderGraph(ppiGraphLayout)
```
...
```{r}
# Load the graph package if not already loaded
library(graph)

# Compute the degree of each node (protein)
node_degrees <- degree(ppiGraph)

# View the raw output
print(node_degrees)

# Sort by highest degree (descending order)
sorted_degrees <- sort(node_degrees, decreasing = TRUE)

# Print the top proteins with the most connections
...
```
More advanced visuals made with chatGPT
```{r}
# Plot histogram of node degrees
hist(sort(node_degrees),
     main = "Degree Distribution of PPI Network",
...
degree_df <- as.data.frame(degree_counts)
colnames(degree_df) <- c("Degree", "Frequency")
...
We found 0 proteins with a degree of 0.
This is expected because the protocol specifically filters for bait proteins, meaning we only measure interactions where at least one prey was bound to a bait.
_______________________________
...
# Extract connected components from the PPI graph
